...
export default class HitlerCommand extends Command {
    constructor(){
        super({
            name: "hitler",
            description: "makes someone more worse than Hitler",
            category: "fun",
            usage: "hitler [@User/User ID]"
        });
    };
    run: CommandRunner = async (client, message, args, prefix) => {
        ...
        return message.channel.send(new MessageAttachment(await canvacord.Canvas.hitler(user.displayAvatarURL({ dynamic: false, format: "png" })), "delete.png"));
    };
};