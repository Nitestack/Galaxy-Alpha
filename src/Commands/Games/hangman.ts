        ...
        message.author.send(client.createEmbed()
            ...
            .setDescription("Now you have to choose a word!\nYou can also use multiple words!\nDM me your word!\n\nYou have 30s to answer!")).then(msg => {
                msg.channel.awaitMessages(m => m.author.id == message.author.id, { time: 30000, max: 1 }).then(collectedMessages => {
                    if (collectedMessages.size == 0) return message.channel.send(client.createRedEmbed(true, `${prefix}${this.usage}`)
                        .setTitle("😩 Hangman Manager")
                    ...
                    message.channel.awaitMessages(m => m.author.id == message.author.id, { time: 30000, max: 1 }).then(collectedUser => {
                        if (collectedUser.first().content.toLowerCase() == 'all' || collectedUser.first().content.toLowerCase() == 'max') {
                            ...
                            users.forEach(user => {
                                if (message.guild.members.cache.has(user)) if (user != message.author.id) validUsers.push(user);
                            });
                            if (collectedUser.first().mentions.users.first()) collectedUser.first().mentions.users.forEach(userMention => {
                                if (message.guild.members.cache.has(userMention.id)) if (userMention.id != message.author.id && !validUsers.includes(userMention.id)) validUsers.push(userMention.id);
                            });
                            if (validUsers.length == 0) return message.channel.send(client.createRedEmbed(true, `${prefix}hangman`)
                                .setTitle("😩 Hangman Manager")
                        ...
                        function nextLetter(message: Message, index: number, word?: string) {
                            message.react(letters[index]).then(r => {
                                ...
                                if (index < letters.length) {
                                    if (index == 13) message.channel.send(generateMessage(word, [])).then((m) => {
                                        games.push({
                                            stage: 0,
                                            msg0: message,
                                            msg1: m,
                                    ...
                                    else nextLetter(message, index, word);
                                };
                            });
                        };
                                    ...
                                    if ((msg.id == game.msg0.id || msg.id == game.msg1.id) && game.stage < stages.length) {
                                        ...
                                        reaction.users.fetch().then((usrs) => {
                                            ...
                                            var remove_next = function (index: number) {
                                                if (index < reactors.length) reaction.remove().then(() => remove_next(index + 1));
                                            };
                                            remove_next(0);
                                        });
                                        if (game.guesses.indexOf(letter) == -1) {
                                            ...
                                            if (game.phrase.indexOf(letter) == -1) {
                                                ...
                                                for (var j = 0; j < game.phrase.length; j++) {
                                                    var c = game.phrase[j];
                                                    if (c != " " && game.guesses.indexOf(c) == -1) sik = false;
                                                };
                                                if (sik) game.msg0.edit(stages[game.stage].replace("o", "o ~ ur alright.. for now"));
                        ...
                        message.channel.send(stages[0]).then(m => nextLetter(m, 0, word));
                    });
                });
            });
    };
